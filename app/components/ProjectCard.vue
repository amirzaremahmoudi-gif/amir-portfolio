  ...
  >
    ...
    >
      <div class="project-media relative overflow-hidden bg-[color:var(--portfolio-surface)]">
        ...
        >
          <span class="project-placeholder-label absolute top-5 text-[10px] font-bold uppercase tracking-[0.2em] text-black/60 md:top-7">{{ t('project.imagePlaceholder') }}</span>
          <span class="project-placeholder-number editorial-display absolute bottom-4 text-[clamp(4.5rem,12vw,10rem)] text-black/12 md:bottom-6">{{ String(index + 1).padStart(2, '0') }}</span>
        </div>
        <span class="project-action absolute bottom-4 right-4 inline-flex items-center gap-2 rounded-full bg-[color:var(--portfolio-bg)] px-4 py-2 text-xs font-semibold shadow-lg md:bottom-5 md:right-5">
          ...
          <UIcon
            ...
            class="project-action-icon size-4"
            aria-hidden="true"
          />
        </span>
      </div>
      <div class="project-copy grid gap-4 border-b border-default py-5 transition-transform duration-[var(--motion-control)] ease-[var(--ease-standard)] sm:grid-cols-[minmax(0,1fr)_auto] md:py-6">
        ...
        <span class="hidden self-end text-xs font-semibold text-muted transition-colors group-hover:text-highlighted group-focus-within:text-highlighted sm:block">{{ t('project.open') }}</span>
      </div>
    </NuxtLink>
  </article>
</template>
...
.project-placeholder-label { inset-inline-start: 1.25rem; }
.project-placeholder-number { inset-inline-end: 1.25rem; }
...
[dir='rtl'] .project-action-icon { transform: scaleX(-1); }
...
[dir='rtl'] .project-card:hover .project-copy, [dir='rtl'] .project-card:focus-within .project-copy { transform: translateX(-.35rem); }
